...
let ipfsAPI = ''; // API key for ntf.storage used for ipfs upload
let displayDevMenu = false; // Displays the SCP Developer button in the header menu.

function setValueSetting(evt) {
  if (!evt) return;
  const value = evt.value;
  const strSetting = evt.getAttribute('setting');
  if (strSetting === 'ipfsApiKey') {
    ipfsAPI = value ? value : '';
    localStorage.setItem(strSetting, value);
  }
}
...
function loadSettings() {
  ...
  for (const dbKey of Object.keys(localStorage)) {
    ...
    if (dbKey === 'ipfsApiKey') {
      ipfsAPI = dbValue;
      if (!!ipfsAPI) {
        document.getElementById('ipfsApiKeySetting').value = ipfsAPI;
      }
    }
    // Set the UI element
    const domSwitch = document.getElementById(dbKey + 'Setting');
    if (domSwitch) domSwitch.checked = boolVal;
  }
}
