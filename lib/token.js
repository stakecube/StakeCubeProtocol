...
class SCP1Token {
    ...
    transfer(acc1, acc2, amount) {
        ...
        if (!this.debitAccount(acc1, amount)) return;
        // Credit the tokens to the second account
        this.creditAccount(acc2, amount);
...
class SCP2Token extends SCP1Token {
    ...
    getStakingStatus(address) {
        ...
        if (cAccount) {
            ...
            if (nAge >= this.minAge) {
                ret.enabled = true;
                ret.note = "currently staking " + (cAccount.balance / COIN).toLocaleString('en-GB') + " " + this.ticker + " with an age of " + nAge + " blocks";
            } else {
    ...
    creditAccount(address, amount, tx) {
        // Ensure appended credit does not exceed the maximum supply
        if ((this.getSupply() + amount) > this.maxSupply) {
            console.error("SCP-2: Attempted credit of '" + amount + "' for token '" + this.name + "' exceeeds maximum supply of '" + this.maxSupply + "'!");
            return false;
        }
        // Search for an already-existing acccount
        let accFound = false;
        ...
        console.log("SCP-2: Issuer for token '" + this.name + "' minted '" + amount + " " + this.ticker + "', new supply is '" + this.getSupply()  + " " + this.ticker + "'!");
        return true;
    }
    ...
    debitAccount(address, amount, tx) {
        // Ensure expended debit does not bring the supply into the negative
        if ((this.getSupply() - amount) < 0) {
            console.error("SCP-2: Attempted burn of '" + amount + "' for token '" + this.name + "' brings the supply into the negative!");
            return false;
        }
        // Search for an already-existing acccount
        for (const account of this.owners) {
            if (account.address === address) {
                ...
                if ((account.balance - amount) >= 0) {
                    ...
                    console.log("SCP-2: User for token '" + this.name + "' burned '" + amount + " " + this.ticker + "', new balance is '" + account.balance + "', new supply is '" + this.getSupply()  + "'!");
                } else {
                    console.log("SCP-2: Attempted burn of token '" + this.name + "' of amount '" + amount + " " + this.ticker + "' failed due to insufficient funds!");
                    return false;
                }
            }
        }
    ...
    transfer(acc1, acc2, amount, tx) {
        ...
        console.log("SCP-2: User for token '" + this.name + "' transferred '" + amount + " " + this.ticker + "' to another account!\nFrom: (" + acc1 + "), To: (" + acc2 + ")");
        return true;
    }
}
