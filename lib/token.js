...
function tokenTick() {
    ...
    for (const cToken of stateTokens) {
        // Update supply cache for all tokens
        cToken.calcSupply();
        if (cToken.version === 2) {
            // Loop all stakers
...
class SCP1Token {
    ...
    // Calculate the current supply of the SCP-1 token, and update the state cache
    calcSupply() {
        let nSupply = 0;
        for (const account of this.owners) {
            nSupply += account.balance;
        }
        // Update cache & return the value
        this.supply = nSupply;
        return nSupply;
    }
...
class SCP2Token extends SCP1Token {
    ...
    creditRewards(address) {
        ...
        let nWeight = this.getWeightPercent(address);
        ...
        if ((this.supply + nReward) >= this.maxSupply) return false;
        // Credit the reward
