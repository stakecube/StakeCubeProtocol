...
class SCP2Token extends SCP1Token {
    ...
    getStakingStatus(address) {
        const ret = {
            ...
            'note': 'this address does not inherit an account for this SCP-2' +
                    'token'
        };
        ...
        if (cAccount) {
            ...
            if (nAge >= this.minAge) {
                ret.enabled = true;
                const strBal = (cAccount.balance / COIN);
                const strPendingBal = (cAccount.unclaimed_balance / COIN);
