...
async function processState(newMsg, tx) {
    ...
    if (newMsg.startsWith("SCPCREATE ") && !_.isEmpty(tx) && !_.isEmpty(tx.txid)) {
            ...
            if (check1 && check2 && check3) {
                ...
                if (tx.vin && !_.isEmpty(tx.vin) && tx.vin[0] && !_.isEmpty(tx.vin[0].txid)) {
                    try {
                        ...
                        let newToken = new SCPToken(tx.txid, arrParams[1], arrParams[2], check3_num, addrCreator);
                        stateTokens.push(newToken);
        ...
        */
        ...
        // TODO: Implement SCP contract write operations (Mints and Transfers)
    } else {
...
// SCP Tokens
class SCPToken {
    constructor(contract, name, ticker, maxSupply, creator, utxos = []) {
