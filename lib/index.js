...
async function getMsgsFromChain(nBlocksTotal) {
    ...
    for (let i=nStartBlock; i<nBestBlock.height+1; i++) {
        // TODO: This can be optimized and sped-up by removing the 'getblockhash' call, by supplimenting it
        // ... for the 'nextblockhash' result of 'getblock' called in getMsgsFromBlock.
        let currentBlock = await SCC.call("getblockhash", i);
        // Skip blocks if we've already scanned them before
        if (chainHashesCache.includes(currentBlock)) continue;
...
async function getMsgsFromBlock(blk) {
    ...
    for (let i=0; i<res.nTx; i++) {
      ...
      chainMessages.push({msg: txRes.msg, time: res.time, tx: res.tx[i], mempool: false});
...
async function processState(newMsg, tx) {
    ...
    if (newMsg.startsWith("SCPCREATE ") && !_.isEmpty(tx) && !_.isEmpty(tx.txid)) {
            ...
            if (check1 && check2 && check3) {
                ...
                if (tx.vin && !_.isEmpty(tx.vin) && tx.vin[0] && !_.isEmpty(tx.vin[0].txid)) {
                    try {
                        ...
                        let newToken = new TOKENS.SCPToken(tx.txid, arrParams[1], arrParams[2], check3_num, addrCreator);
                        TOKENS.addToken(newToken);
                        console.log("New SCP token created!");
                        console.log(newToken);
                    } catch (e) {
                        console.warn("An attempt to create a new token has failed, failed to retrieve VIN info!");
                    }
                } else {
                    // This shouldn't EVER happen!
                    console.warn("An attempt to create a new token has failed, missing VIN!");
                }
            } else {
                console.warn("An attempt to create a new token has failed, incorrect params!");
            }
        } else {
            console.warn("An attempt to create a new token has failed, incorrect param count! (has " + arrParams.length + ", expected 4)\nTX: " + tx.txid);
        }
        ...
        */
        ...
        if (check1 && check2) {
            // TODO: Implement SCP contract write operations (Transfers and Burns)
            // Fetch the contract being called
            // SCP-1 Token Contracts
            let cToken = TOKENS.getToken(arrParams[0]);
            if (!cToken.error) {
                // SCP-1 MINTING (Create new tokens on-demand, issuer-only, cannot mint above predefined max supply)
                if (arrParams[1] === "mint") {
                    /*
                    ...
                    */
                    ...
                    let check3 = (check3_num > 0 && Number.isSafeInteger(check3_num));
                    if (check3) {
                        // Fetch VIN --> Grab change output --> Ensure change output is the token issuer
                        let addrCreator = (await SCC.call('getrawtransaction', tx.vin[0].txid, 1)).vout[1].scriptPubKey.addresses[0];
                        if (!addrCreator || _.isEmpty(addrCreator)) throw "Missing creator address!";
                        // Check the change output against the token issuer
                        if (addrCreator === cToken.creator) {
                            // Authentication successful, minting tokens!
                            cToken.creditAccount(cToken.creator, check3_num);
                        } else {
                            console.error("An attempt by a non-issuer to mint SCP-1 tokens failed! (Issuer: " + cToken.creator.substr(0, 5) + "... Caller: " + addrCreator.substr(0, 5) + "...)");
                        }
                    } else {
                        console.error("An attempt to mint SCP-1 tokens failed! (Token: " + cToken.name + ", amount: " + arrParams[2] + ")");
                    }
                }
                // SCP-1 BURNING (Burn tokens from your account balance, usable by anyone, cannot burn more than your available balance)
                    ...
                    */
                    ...
                    let check3 = (check3_num > 0 && Number.isSafeInteger(check3_num));
                    if (check3) {
                        // Fetch VIN --> Grab change output --> Ensure change output is the contract caller
                        let addrCaller = (await SCC.call('getrawtransaction', tx.vin[0].txid, 1)).vout[1].scriptPubKey.addresses[0];
                        if (!addrCaller || _.isEmpty(addrCaller)) throw "Missing caller address!";
                        // Authentication successful, burning tokens!
                        cToken.debitAccount(addrCaller, check3_num);
                    } else {
                        console.error("An attempt to burn SCP-1 tokens failed! (Token: " + cToken.name + ", amount: " + arrParams[2] + ")");
                    }
                }
                // SCP-1 SENDS (Transfer tokens to another account, usable by anyone, cannot send more than your available balance)
                    ...
                    */
                    ...
                    if (check3 && check4 === true) {
                        ...
                        if (!addrCaller || _.isEmpty(addrCaller)) throw "Missing caller address!";
                        // Authentication successful, burning tokens!
                        cToken.transfer(addrCaller, arrParams[3], check3_num);
                    } else {
                        console.error("An attempt to transfer SCP-1 tokens failed! (Token: " + cToken.name + ", from " + addrCaller.substr(0, 5) + ", to " + arrParams[3].substr(0, 5) + ", amount: " + arrParams[2] + ")");
                    }
                }
            } else {
            ...
            console.log("Long non-contract and/or malformed write detected (" + newMsg.length + " bytes), skipping!");
        }
    } else {
        console.log("No contract writes here!");
    }
    ...
    if (!isScanningBlocks) {
        try {
            ...
            if (chainHashesCache.length === 0) {
                // TODO: Make this a bit smarter and scan from a specific 'starter block', or the state will desync in ~20k blocks from this commit!
                await getMsgsFromChain(40000);
            }
