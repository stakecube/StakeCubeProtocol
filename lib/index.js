...
async function init() {
    ...
    try {
        // Load the wallet DB
        let rawWallet = await DB.getWallet();
        if (rawWallet) {
            WALLET.setKeys(rawWallet.pubkey, rawWallet.privkeyDecrypted, rawWallet.privkeyEncrypted);
            WALLET.set2FAkey(rawWallet.opt2FA);
        }
...
async function getFullMempool() {
    let arrFullMempool = [];
    let arrMempool = await SCC.call("getrawmempool");
    for (const cTX of arrMempool) {
        arrFullMempool.push(await SCC.call("getrawtransaction", cTX, 1));
    }
