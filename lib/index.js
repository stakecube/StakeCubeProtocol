...
app.get('/api/v1/getactivitybytxid/:txid/:type', function(req, res) {
    if (!req.params.txid || req.params.txid.length !== 64) {
        return res.json({ 'error': "You must specify a 'txid' param!" });
    }
    const allowedTypes = ['sent', 'received', 'staked'];
    if (req.params.type && !allowedTypes.includes(req.params.type)) {
        return res.json({ 'error': "Bad 'type' (" + req.params.type + ')!' });
    }
    const cLinearActivity = [];
    const strTx = req.params.txid;
    const strType = req.params.type || false;
    // Loop every token
    const cTknPtr = TOKENS.getTokensPtr();
    for (const cToken of cTknPtr) {
        // Loop every account
        for (const cAccount of cToken.owners) {
            ...
            for (const activity of cAccount.activity) {
                ...
                if (strType && activity.type !== strType) continue;
