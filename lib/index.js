...
app.get('/api/v1/wallet/getbalances/:address', async function(req, res) {
    ...
    try {
        ...
        const cTokens = isFullnode ? TOKENS.getTokensByAccount(strAddr) : JSON.parse(await NET.getLightTokensByAccount(strAddr));
...
app.get('/api/v1/wallet/send/:address/:currency/:to/:amount', async function(req, res) {
    ...
    try {
        ...
        if (!cSelectedToken) {
            ...
            // Inputs
            const usedUTXOs = WALLET.getCoinsToSpend(nAmount * COIN, false, cWallet.getPubkey());
            ...
            // Add input
            const cUTXO = WALLET.getCoinsToSpend(10000, true, cWallet.getPubkey())[0];
            cTx.addinput(cUTXO.id, cUTXO.vout, cUTXO.script);
            // SCP output
...
async function init() {
    ...
    try {
        ...
        try {
            const uptime = await RPC.call('uptime');
...
async function getFullMempool() {
    ...
    const arrMempool = await RPC.call('getrawmempool');
            ...
            if (!cToken.error) {
                    ...
                    if (check3) {
                        ...
                        if (addrCreator === cToken.creator) {
                            ...
                            for (const cVin of tx.vin) {
                                ...
                                const vinTx = await RPC.call('getrawtransaction', cVin.txid, 1);
                ...
                // SCP SENDS (Transfer tokens to another account, usable by anyone, cannot send more than your available balance)
                    ...
                    */
                    ...
                    const check3_num = Number(arrParams[2]);
                    const check3 = (check3_num > 0 && Number.isSafeInteger(check3_num));
                    const check4 = (await RPC.call('validateaddress', arrParams[3])).isvalid;
    ...
    if (!isScanningBlocks) {
        try {
            ...
            if (chainHashesCache.length === 0) {
                ...
                const nCurrentBlock = await RPC.call('getblockcount');
