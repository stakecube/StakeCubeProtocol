...
function setKeys(pubkey = false, privkey = false, privkeyEnc = false) {
    if (pubkey)
        pubkeyMain  = pubkey;
    if (privkey)
        privkeyMain = privkey;
...
function getPrivkeyEnc() { return privkeyEncrypted; }

// Formats the wallet database for DB storage
function toDB() {
    return {
        pubkey: getPubkey(),
        privkeyDecrypted: getPrivkeyEnc().length > 0 ? "" : getPrivkey(),
        privkeyEncrypted: getPrivkeyEnc(),
        opt2FA: get2FAkey()
    }
}
...
exports.getPrivkeyEnc     = getPrivkeyEnc;
exports.toDB              = toDB;
