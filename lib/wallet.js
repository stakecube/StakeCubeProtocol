...
async function refreshUTXOs(address = String()) {
    ...
    let res = JSON.parse(await NET.getLightUTXOs(address));
    // Convert explorer dataset into UTXO classes and merge with the current set
    let arrUTXOList = [];
    for (const rawUTXO of res) {
        arrUTXOList.push(new UTXO(address, rawUTXO.txid, rawUTXO.outputIndex, rawUTXO.script, rawUTXO.satoshis));
    }

    // Step 2 --- Mempool Sync
    let rawMempool = [];
    try {
        rawMempool = JSON.parse(await NET.getMempoolLight());
    } catch(e) {
        // Data source error, silently catch to allow the Blockchain sync to succeed at minimum
    }
    ...
    // Search all VOUTs belonging to us, add them to our wallet
        for (const rawVout of rawTX.vout) {
            if (rawVout.scriptPubKey && rawVout.scriptPubKey.addresses && rawVout.scriptPubKey.addresses.length > 0) {
                ...
                const cWallet = getWallet(rawVout.scriptPubKey.addresses[0]);
                if (!cWallet) continue;
                // Found a mempool vout for our wallet!
                let cUTXO = new UTXO(cWallet.getPubkey(), rawTX.txid, rawVout.n, rawVout.scriptPubKey.hex, rawVout.valueSat);
                cUTXO.mempool = true;
    ...
    // Search for all spent VINs for our wallet, and mark them as spent
        for (const rawVin of rawTX.vin) {
            let spentVout = getUTXO(rawVin.txid, rawVin.vout);
            if (spentVout) {
