...
function get2FAkey() { return opt2FA; }
...
class Wallet {
    ...
    getPrivkey() { return this.privkey; }
    // Returns the loaded encrypted privkey
    getPrivkeyEnc() { return this.privkeyEnc; }

    // Formats the wallet database for DB storage
    toDB() {
        return {
            pubkey: this.getPubkey(),
            privkeyDecrypted: this.getPrivkeyEnc().length > 0 ? "" : this.getPrivkey(),
            privkeyEncrypted: this.getPrivkeyEnc()
        }
    }
}
...
function deepCloneWallet(cWallet = new WALLET) {
    return new Wallet(cWallet.getPubkey(), cWallet.getPrivkey(), cWallet.getPrivkeyEnc());
}

// Finds a wallet, by it's address, privkey or privkeyEnc, and if it exists
function getWallet(query = new String) {
    for (const cWallet of arrWallets) {
        if (cWallet.getPubkey()     === query ||
            cWallet.getPrivkey()    === query ||
            cWallet.getPrivkeyEnc() === query) {
                return cWallet;
        }
    }
    return false;
...
function addWallet(cNewWallet = new Wallet) {
    if (getWallet(cNewWallet.getPrivkey()) ||
        ...
        getWallet(cNewWallet.getPrivkeyEnc())) {
            return false;
    }
    arrWallets.push(deepCloneWallet(cNewWallet));
    console.log("Loaded new wallet! Total of " + arrWallets.length + " loaded.");
...
async function createWallet() {
    let rawWallet = await sccjs.generateWallet();
...
function getCoinsToSpend(sats = 0, min = false) {
    let spent = 0;
    let chosenUTXOs = [];
    let suitableUTXOs = getAvailableUTXOs();
    // Goal: Loop all UTXOs until the 'spent' amount is >= the sats amount, plus estimated fees
    for (const cUTXO of suitableUTXOs) {
        // If a minimum is enforcd, ONLY accept UTXOs above the minimum
        if (min && cUTXO.sats <= sats) continue;
        if (spent >= (sats + 1000)) {
            console.log("Coin Control: TX Constructed! Selected " + chosenUTXOs.length + " input(s) (" + (spent / COIN) + " SCC)");
            return chosenUTXOs;
        }
        spent += cUTXO.sats;
